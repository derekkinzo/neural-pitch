...
//!
//! [`ContourSmoother`] maintains a sliding-window running mean over cents
//! (relative to a sliding-window reference), parameterised by `window_ms`
//! and `sample_rate_hz`. Unvoiced frames are passed through untouched.
...
impl ContourSmoother {
    ...
    /// # Capacity caveat
    ///
    /// `capacity == window_ms.max(1.0).ceil() as usize`, i.e. one frame
    /// per millisecond. That is correct only when the analyzer's frame
    /// rate is exactly 1000 Hz. For the live path (hop=512 @ 48 kHz =>
    /// ~93.75 Hz frame rate) and the offline pYIN path (hop=256 @ 48 kHz
    /// => ~187.5 Hz) the *effective* smoothing window in real time is
    /// several times longer than `window_ms` would suggest. Treat
    /// `window_ms` as a *frame budget* (in milliseconds-of-frames)
