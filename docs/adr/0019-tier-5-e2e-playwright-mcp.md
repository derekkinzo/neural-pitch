...
- **`@playwright/mcp` 0.0.75** is configured under `tests/mcp/` as the agent-driven exploratory layer. It is **not** part of the deterministic CI gate; it is invoked ad hoc by Claude or an engineer for exploratory walkthroughs against the same Vite dev server with the same mock bridge enabled.
